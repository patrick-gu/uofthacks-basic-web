    ...
    while (ctx.ip < prog.statements.length) {
        ...
        switch (stmt.type) {
            ...
            case "attribute": {
                ...
                if (tag.type === "root") {
                    throw new Error(
                        "Can only add attributes when inside a tag."
                    );
                }
            ...
            case "gotoIf": {
                ...
                if (cond.type === "boolean") {
                    ...
                    throw new Error(
                        `Cannot write condition of type ${stmt.type}.`
                    );
                }
                break;
            }
...
function arrayIndex(
    ...
    if (index.type !== "number") {
        throw new Error(
            `Cannot index array with non-number type ${index.type}`
        );
    }
    if (
...
function renderNode(
    ...
    if (node.type === "tag") {
        ...
        res.replaceChildren(
            ...node.content.map((n) => renderNode(prog, ctx, n))
        );
        return res;
    } else {
        return node.value;
